...
- **AI Capability**: AI text, image, video, audio generation, and speech transcription via the aihub module. Covers model selection, frontend `client.ai` / `client.apiCall.invoke` usage, backend `AIHubService` usage, and the distinction between direct aihub endpoints and custom wrapper APIs.
